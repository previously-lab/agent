...
export function RepoHub() {
  ...
  const handleAdd = async () => {
    ...
    if (!parsed) {
      setError("Enter a valid GitHub URL or owner/repo (e.g. github.com/owner/repo)");
      return;
    }
    ...
    <div className="space-y-6">
      <div>
        ...
        {vercelRepo && (
          <div className="flex items-center gap-3 rounded-lg border border-border bg-muted/30 p-3 mb-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-muted-foreground">Primary (Vercel)</div>
            </div>
        ...
        {repos.length === 0 && !vercelRepo && (
          <p className="text-sm text-muted-foreground py-2">No repositories connected. Add one below.</p>
        )}
        ...
        {repos.map((r) => (
          <div key={r.id} className="flex items-center gap-3 rounded-lg border border-border p-3 mb-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-muted-foreground">Added {new Date(r.addedAt).toLocaleDateString()}</div>
            </div>
            <Button variant="ghost" size="icon" onClick={() => handleRemove(r.id)} className="h-8 w-8 text-muted-foreground hover:text-destructive" title="Remove repository">
      ...
      <div className="border-t border-border pt-4">
        <h4 className="text-sm font-medium mb-2">Add Repository</h4>
        ...
        <p className="text-xs text-muted-foreground mt-1.5">
          Add repositories your agent can access. Each repo will be registered in your memory.
        </p>
      </div>
    </div>
  );
