  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className={cn(
            "min-w-0 shrink truncate font-light leading-none",
            showErrorHeader
              ? "text-red-500"
              : showInterruptedHeader
                ? "text-yellow-500"
        ...
        <div className="flex min-w-0 flex-1 items-baseline gap-1.5 overflow-hidden">
          ...
          {showErrorHeader && hasErrorMeta && (
            <span className="inline-flex shrink-0 items-center gap-1.5 text-[12px] font-light leading-none text-red-400/70">
              {errorMeta}
            </span>
          )}
          {hasTrailingMeta && (
            <span className="inline-flex shrink-0 items-center gap-1.5 text-[12px] font-light leading-none text-muted-foreground">
              {meta}
            </span>
          )}
        </div>
      ...
      <div className="px-3">
        ...
        {hasOutput && !state.interrupted && (
          <div className="mt-2 text-xs font-light text-muted-foreground">{output}</div>
        )}

        {state.denied && (
          <div className="mt-2 text-xs font-light text-red-500">
                ...
                <div className="pt-1.5 pb-1">
                  ...
                  <Card size="sm" className="ring-inset">
                    <CardContent className="max-h-80 overflow-auto text-xs">
                      ...
                      {isInterrupted && (
                        <p className="text-xs font-light leading-relaxed text-yellow-500">
                          {t("interrupted")}
                        </p>
                      )}
                      {expandedContent}
