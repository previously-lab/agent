...
/**
 * The conversation surface — a DOM scroll container with windowed mounting.
 *
 * THIS USED TO BE A CAMERA. Until v0.11 the conversation rendered inside an
 * R3F canvas (`conversation-field.tsx`, deleted): every block a billboard,
 * the scroll position a camera offset the field owned. The product decision
 * (design doc §13/§14.5) is that R3F renders what has ALREADY HAPPENED — the
 * closed time slices — and the conversation in flight is plain DOM: selectable,
 * copyable, readable by a screen reader, scrolled by the browser's own wheel,
 * touch and keyboard handling. This file is that surface.
 *
