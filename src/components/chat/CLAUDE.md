...
The home page is a single shell — the left time axis is persistent, and the right pane shows the card field at a card rung. There is no view mode: `?z=<rung>` names the rung (absent = `conversation`) and `src/lib/chat/deep-link.ts` owns the parsing. The card field (`src/components/timeline-3d/`) renders inside the shell's right pane at slice/day/week; the header's segmented pill is gone and the board bar (`shell/board-bar.tsx`, top centre) is the only control that moves along the ladder — it holds the zoom lens and the strand selector, which used to sit on the 24-32px time rail. The conversation is no longer a pane at all: it is the CONVERSATION LAYER (`conversation-panel.tsx`), a persistent three-tier overlay (pill / dock / fullscreen, v0.11 §14.1) that floats over the world — `position: fixed`, so opening it never resizes the canvas (覆盖，不挤压), and fullscreen freezes the world's `frameloop` without unmounting it. The stream stays mounted at every tier (pill slides the panel offscreen + `inert`), so its scroll position and the live useChat stream survive every collapse. The shell pins the panel's ChatPage to the `conversation` rung, so the composer's COMPACT form (`composer-host.tsx` + `chat-input.tsx`'s `collapsed`) is currently latent — opening the panel IS the compact composer's successor.
