...
The conversation is ALSO the CONVERSATION LAYER (`conversation-panel.tsx`, v0.11 §14.1): a persistent three-tier overlay (pill / dock / fullscreen) that floats over the world — `position: fixed`, so opening it never resizes the canvas (覆盖，不挤压), and fullscreen freezes the world's `frameloop` without unmounting it. The shell pins the panel's ChatPage to the `conversation` rung (tier visibility is the panel's job), so the composer's COMPACT form (`composer-host.tsx` + `chat-input.tsx`'s `collapsed`) is latent — the panel hosts the composer.

**THE 2.5D SPLIT (the 2026-09 restore).** The R3F conversation field (`conversation-field.tsx`, deleted by `419ad3d`) is restored, and it once again renders the conversation rung IN THE PANE — history, slice gates, the older-page head, the camera, all of it verbatim. The field is authored against the window-derived tier column (680 px) and cannot fit the panel's 420–520 px dock, so it reaches the pane through a portal: `unified-chat-stream.tsx` renders `ConversationField` into a slot the shell owns (`conversation-surface.tsx`) — the pane slot while the panel floats beside it, a slot inside the panel body at fullscreen. Where no wide surface exists (the game's docked/pilled panel) the DOM list (`dom-chat-list.tsx`, the pre-restore surface) carries the conversation instead. **The ONE difference from the old field: the turn that is IN FLIGHT never enters the 3D field** — history is the field's whole content, and the ongoing turn renders as plain DOM in the panel, above the composer (`LiveTurnStrip`). The panel is the ongoing conversation's home and the only input.
...
└── ConversationPanel (conversation-panel.tsx — the three tiers; hosts ChatPage)
...
On narrow surfaces (the game's docked/pilled panel) the adapter renders `DomChatList` instead — the whole stream, history and live, as a native scroll container (`stream-layout.ts` arithmetic).
...
| `conversation-panel.tsx` | **The conversation layer's three tiers** (v0.11 §14.1): pill / dock / fullscreen, controlled by the owning surface (`app-shell`), which reacts to `mode` (per-surface default tier, `frameloop` freeze at fullscreen). Pure `reducePanelMode` transition table (unit-tested in `__tests__/`), `Cmd/Ctrl+J` toggle (K = search, `.` = rungs were taken), container-level `Escape` (portaled popovers never reach it), focus follows the tier. The children stay MOUNTED at every tier — pill is a slide-offscreen, not an unmount. At fullscreen its `bodyPrefix` slot hosts the R3F field |
...
- **Why the field portals instead of mounting in place.** The field's reading column comes from `useTier()` — a WINDOW-derived 680 px, agreed with the card field's conversation units so a slice never reflows on a rung change. The conversation panel's dock is 420–520 px: the column cannot fit it, and shrinking the column would break the agreement. So the shell owns a surface slot (`conversation-surface.tsx`, the same provider pattern as `world-slot.tsx`): the field renders in the pane at the conversation rung, in the panel body at fullscreen, and the DOM list covers surfaces with no wide host. One `useChat`, one composer, both surfaces mounted through the panel.
