 ...
 * the card renders. No cache, no store: the catalog is already the cheap
 * derived index, and the demo backend caches at the fs layer.
 *
...
export interface HomeRecap {
  ...
  timezone: string;
  focus: string;
  /** The reader's last line, truncated. Null when the slice has none. */
  lastUser: string | null;
  /** Previously's last line, truncated. Null when the slice has none. */
...
/** Collapse to one line and cut at a word boundary near the limit. */
function quote(turn: Turn | undefined): string | null {
  if (!turn) return null;
  const flat = turn.content.replace(/\s+/g, " ").trim();
  if (!flat) return null;
  if (flat.length <= QUOTE_CHARS) return flat;
  const cut = flat.slice(0, QUOTE_CHARS);
  const lastSpace = cut.lastIndexOf(" ");
  const base = (
    lastSpace > QUOTE_CHARS * 0.6 ? cut.slice(0, lastSpace) : cut
  )
    .trimEnd()
    .replace(/[.,;:!?，。；：！？、…—-]+$/, "");
  return `${base}…`;
